...
import { Provider, responseAsToolCall, toLLMTools } from "../models";
import { OpenAIProviderOptions as BaseOpenAIProviderOptions } from "./openai";
    ...
    chat: async ({ messages, response_format, temperature, ...options }) => {
      ...
      if (Object.keys(response_format).includes(firstToolCall.function.name)) {
        ...
        if (message.tool_calls.length > 1) {
          throw new Error(
            `When calling one of ${Object.keys(response_format).join(
              ", "
            )}, you cannot call other tools.`
          );
        }
