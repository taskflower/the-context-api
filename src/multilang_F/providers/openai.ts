...
import { Provider, responseAsStructuredOutput, toLLMTools } from "../models";
...
export const openai = (options: OpenAIProviderOptions = {}): Provider => {
  ...
  return {
    chat: async ({ messages, response_format, temperature, ...options }) => {
      ...
      const tokenUsage = response.usage?.total_tokens; // Zapisujemy ilość tokenów
    
      if (message.tool_calls.length > 0) {
        return {
          type: "tool_call",
          value: message.tool_calls,
          tokens: tokenUsage, // Dodajemy tokeny do odpowiedzi
        };
      }
      ...
      return { ...message.parsed.response, tokens: tokenUsage }; // Dodajemy tokeny
    }
