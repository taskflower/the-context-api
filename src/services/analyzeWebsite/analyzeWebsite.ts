// src/services/analyzeWebsite/analyzeWebsite.ts
import { Request, Response } from 'express';
import { websiteAnalysisService } from './analyzeWebsite.config';
...
export async function handleAnalyzeWebsite(req: Request, res: Response): Promise<void> {
    ...
    try {
        const result = await websiteAnalysisService.analyzeWebsite({ url, userId });
        res.status(200).json({
